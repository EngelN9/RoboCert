> **Historical — superseded on 2026-09-24 by [`ROADMAP.md`](../../ROADMAP.md).** This is the original `ROADMAP.md`, kept verbatim because ledger entries, frozen proof-verification runs, and attested formal sources cite its section numbers. A reference of the form `ROADMAP.md §N` or `ROADMAP.md Phase N` written before 2026-09-24 means this file. Nothing here describes a current capability or the current plan. For current status, see the [README](../../README.md) and [`research/CLAIMS.md`](../../research/CLAIMS.md).
